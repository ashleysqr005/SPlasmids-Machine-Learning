...
"""def collect_rows(df, ko_emb, target, habitat_enc=None):
    rows = []
    for _, row in df.iterrows():
        ko = row["KO"]
        if ko not in ko_emb or pd.isna(row[target]):
...
"""def plot_regression(y_true, y_pred, metrics: dict, name: str):
    plt.figure(figsize=(5, 5))
    sns.scatterplot(x=y_true, y=y_pred, alpha=0.5, s=15)
    lo = min(y_true.min(), y_pred.min())
    hi = max(y_true.max(), y_pred.max())
...
def run_baselines(X_tr_s, X_te_s, y_tr, y_te, n_emb):
    """
    #Four baselines evaluated on the same train/test split as the MLP.

    #n_emb: number of embedding dimensions (1280 for ESM-2 650M).
           #The remaining columns are the habitat one-hot encoding.

    #Returns a list of dicts for easy CSV export.
"""
    print(f"\n{'='*50}")
    print("BASELINES")
    print(f"{'='*50}")
    ...
    m = evaluate(y_te, dummy.predict(X_te_s))
    print(f"{'Dummy (mean)':<25} {m['r2']:>7.3f} {m['spearman_r']:>11.3f}")
    results.append({"model": "dummy_mean", **m})

    # 2. Habitat one-hot only (no embedding)
    ridge_hab = Ridge()
    ridge_hab.fit(X_tr_s[:, n_emb:], y_tr)
    m = evaluate(y_te, ridge_hab.predict(X_te_s[:, n_emb:]))
    print(f"{'Ridge (habitat only)':<25} {m['r2']:>7.3f} {m['spearman_r']:>11.3f}")
    results.append({"model": "ridge_habitat_only", **m})

    # 3. Embedding only (no habitat one-hot)
    ridge_emb = Ridge()
    ridge_emb.fit(X_tr_s[:, :n_emb], y_tr)
    m = evaluate(y_te, ridge_emb.predict(X_te_s[:, :n_emb]))
    print(f"{'Ridge (embedding only)':<25} {m['r2']:>7.3f} {m['spearman_r']:>11.3f}")
    results.append({"model": "ridge_embedding_only", **m})

    # 4. Embedding + habitat, linear
    ridge_all = Ridge()
    ridge_all.fit(X_tr_s, y_tr)
    m = evaluate(y_te, ridge_all.predict(X_te_s))
    print(f"{'Ridge (emb + habitat)':<25} {m['r2']:>7.3f} {m['spearman_r']:>11.3f}")
    results.append({"model": "ridge_emb_and_habitat", **m})
    ...
    print(f"Baseline results saved to {OUT_DIR}/baseline_results.csv")
...
"""def run_finetune(X_tr, X_te, y_tr, y_te, ko_tr, n_emb):
    """
    #Two-phase finetune pipeline for mean__H2_global_b_ko_hab.

    #n_emb: embedding dimension, used to split features for baselines.
    """
    # --- Standardize ---
    x_scaler = StandardScaler()
    X_tr_s   = x_scaler.fit_transform(X_tr)
    ...
    # --- Run baselines first ---
    run_baselines(X_tr_s, X_te_s, y_tr, y_te, n_emb)
    ...
    # --- Phase 1: Hyperparameter search ---
    print(f"\nPhase 1 — hyperparam search ({N_ITER} iterations)...",
          end=" ", flush=True)
    ...
    print(f"\n{'='*50}")
    print(f"RESULTS: mean__H2_global_b_ko_hab (finetuned v2)")
    print(f"{'='*50}")
    print(f"{'Metric':<15} {'Train':>8} {'Test':>8}")
    print(f"{'-'*35}")
    for key in ["r2", "rmse", "mae", "pearson_r", "spearman_r"]:
        print(f"{key:<15} {metrics_tr[key]:>8.3f} {metrics_te[key]:>8.3f}")
    print(f"{'gap (R²)':<15} "
          f"{metrics_tr['r2'] - metrics_te['r2']:>8.3f}")

    plot_regression(y_tr, pred_tr, metrics_tr,
                    "finetune_v2_mean_H2_global_b_ko_hab_train")
    plot_regression(y_te, pred_te, metrics_te,
                    "finetune_v2_mean_H2_global_b_ko_hab_test")
    ...
    mlp_preds_df.to_csv(os.path.join(OUT_DIR, "mlp_preds.csv"), index=False)
    print(f"MLP predictions saved to {OUT_DIR}/mlp_preds.csv")
    ...
    pd.DataFrame([dict(
        ...
        **best_params,
        ...
        os.path.join(OUT_DIR, "finetune_v2_results.csv"), index=False
    )
    print(f"\n✅ Results saved to {OUT_DIR}")"""
...
"""if __name__ == "__main__":
    ...
    #ko_l, X, y = collect_rows(df, ko_emb, TARGET, habitat_enc=h_enc)
    ko_l, X, y = collect_rows(
        df, ko_emb, TARGET,
        habitat_enc=h_enc,
        feature_mode=FEATURE_MODE
    ...
    # Embedding dimension for ESM-2 650M is 1280
    # The remaining columns are the habitat one-hot encoding
    #n_emb = 1280
    emb_dim = next(iter(ko_emb.values())).shape[0]
    hab_dim = len(h_enc.categories_[0])
    ...
    print(f"Train: {len(y_tr)}  Test: {len(y_te)}")
    run_finetune(X_tr, X_te, y_tr, y_te, ko_tr, n_emb=n_emb)"""