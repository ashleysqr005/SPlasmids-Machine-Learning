...
Usage:
  python scripts/project_ko_embeddings.py \
    ...
    --plot-per-habitat --tsne-perplexity 15
...
def prepare_data(
    ...
    Returns:
        ...
        ko_df     : DataFrame with one row per unique KO
                    columns: KO, Level3_clean (first occurrence), mean_b_ko_hab
        hab_pivot : DataFrame (n_ko rows) with one column per habitat,
                    values = b_ko_hab for that KO x habitat (NaN if absent)
    """
    df = raw_df[raw_df["KO"].astype(str).isin(emb.keys())].copy()
    df["KO"] = df["KO"].astype(str)

    # per-habitat b values: deduplicate KO x habitat
    hab_b = (
        df[["KO", "habitat", "b_ko_hab"]]
        .drop_duplicates(subset=["KO", "habitat"])
    )
    # pivot: rows=KO, cols=habitat, values=b_ko_hab
    hab_pivot = hab_b.pivot(index="KO", columns="habitat", values="b_ko_hab")
    # filter out individual data pointw with low b_ko_hab
    min_b_ko_hab = 1.2   # example threshold
    hab_pivot = hab_pivot.where(hab_pivot >= min_b_ko_hab, np.nan)

    hab_pivot.columns.name = None

    # KO-level metadata: one row per KO (take first Level3_clean occurrence)
    ko_meta = (
        df[["KO", "Level3_clean"]]
    ...
    ko_df = ko_meta.reset_index()  # columns: KO, Level3_clean, mean_b_ko_hab

    return x, ko_df, hab_pivot.reset_index(drop=True)
...
def plot_per_habitat(z, ko_df, hab_pivot, method, out_dir):
    ...
    #One PNG per habitat.
    #KOs present in habitat: colored by b_ko_hab (plasma, per-habitat scale).
    ...
    for hab in habitats:
        ...
        if absent.any():
            ax.scatter(z[absent, 0], z[absent, 1],
                       c="lightgrey", s=3, alpha=0.3, linewidths=0, zorder=1)

        vals = b_series[present].values
        vmin, vmax = _b_ko_hab_colormap_limits(vals)
        if vals.size > 0:
            sc = ax.scatter(z[present, 0], z[present, 1],
                            c=vals,
                            s=10, alpha=0.9, linewidths=0,
                            cmap="plasma", vmin=vmin, vmax=vmax, zorder=2)
            fig.colorbar(sc, ax=ax, fraction=0.046, pad=0.04, label="b_ko_hab")

        ax.set_title(f"{method.upper()} — {hab}  (n={int(present.sum())})  |  b_ko_hab")
        ax.set_xlabel("dim 1"); ax.set_ylabel("dim 2")
        fig.tight_layout()

        safe = hab.replace(" ", "_").replace("/", "-")
        fig.savefig(hab_dir / f"{method}_bkohab_{safe}.png", dpi=200, bbox_inches="tight")
        plt.close(fig)

    print(f"  Per-habitat individual plots → {hab_dir}", flush=True)


def plot_per_habitat_grid(z, ko_df, hab_pivot, method, out_dir):
    """Summary grid: one subplot per habitat; each panel has its own colorbar (per-habitat scale)."""
    habitats = list(hab_pivot.columns)
    n        = len(habitats)
    ncols    = min(4, n)
    nrows    = int(np.ceil(n / ncols))

    # Extra width so each subplot's colorbar fits beside the axes.
    fig, axes = plt.subplots(nrows, ncols,
                             figsize=(5.8 * ncols, 4 * nrows),
                             squeeze=False)
    fig.suptitle(f"{method.upper()} — b_ko_hab per habitat", fontsize=14, y=1.01)
    ...
    for idx, hab in enumerate(habitats):
        ...
        if (~present).any():
            ax.scatter(z[~present, 0], z[~present, 1],
        ...
        if vals.size > 0:
            sc = ax.scatter(z[present, 0], z[present, 1],
                            ...
                            cmap="plasma", vmin=vmin, vmax=vmax, zorder=2)
            fig.colorbar(sc, ax=ax, fraction=0.055, pad=0.04, label="b_ko_hab")
    ...
    fig.tight_layout()
    grid_path = out_dir / f"{method}_bkohab_per_habitat_grid.png"
    fig.savefig(grid_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"  Per-habitat grid → {grid_path}", flush=True)
...
def main() -> int:
    ...
    parser.add_argument("--plot-per-habitat", action="store_true",
    ...
    x, ko_df, hab_pivot = prepare_data(raw_df, emb)
    ...
    # per-habitat plots
    if args.plot_per_habitat:
        print("Generating per-habitat plots …", flush=True)
        for method, z in [("umap", z_umap), ("tsne", z_tsne)]:
            if z is None:
                continue
